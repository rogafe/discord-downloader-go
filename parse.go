...
func getTwitterStatusUrls(inputURL string, channelID string) (map[string]string, error) {
	if twitterClient == nil {
		...
		lolmatches := regexUrlTwitterStatus.FindStringSubmatch(inputURL)
		// statusId, err := strconv.ParseInt(matches[4], 10, 64)
		// log.Println(matches)
		scraper := twitterscraper.New()
		tw, err := scraper.GetTweet(lolmatches[4])
		if err != nil {
			return nil, errors.New("error while scrapping")
		}
		// log.Println(tw.Photos)
		for _, photo := range tw.Photos {
			foundUrls := getDownloadLinks(photo, channelID)
			// log.Panicln(foundUrls)
